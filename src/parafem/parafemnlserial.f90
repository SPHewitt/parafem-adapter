  ...
  SUBROUTINE initnlserial(g_num,g_coord,rest,nels,nod,nn,nr,g_num_pp,g_g_pp,g_coord_pp)
  !/****f* parafeml/initnl
  !*  NAME
  !*    SUBROUTINE: initnl
  !*
...
!----------------------------------------------------------------------
  ...
  INTEGER,INTENT(IN) 	    :: nn,nr,nod,nels
  INTEGER,INTENT(IN) 	    :: g_num(nod,nels)
...
!---------------------------------------------------------------------- 
  ...
  IF(numpe .EQ.1) PRINT*,"Point B",npes,nod,nels
...
!----------------------------------------------------------------------
  ...
  SUBROUTINE runnlserial(nels,node,val,num_var,mat_prop,nod,nr,loaded_nodes,timeStep, 	&
...
!------------------------------------------------------------------------------
  ...
  INTEGER,INTENT(IN)        :: loaded_nodes,nr,nod,nels
  ...
  REAL(iwp),INTENT(INOUT)   :: Dfield(ntot,nels),Ufield(ntot,nels)
  REAL(iwp),INTENT(INOUT)   :: Afield(ntot,nels), val(ndim,loaded_nodes)

  REAL(iwp)                 :: up,x_f,y_f,z_f
...
!------------------------------------------------------------------------------
  ...
  Print*,numpe,SUM(val(1,:)),SUM(val(2,:)),SUM(val(3,:))
  ...
  CALL MPI_BARRIER(MPI_COMM_WORLD,ier)
  if(numpe.eq.1)Print*, numpe,x_f,y_f,z_f
  if(numpe.eq.2)Print*, numpe,x_f,y_f,z_f
  if(numpe.eq.3)Print*, numpe,x_f,y_f,z_f
  if(numpe.eq.4)Print*, numpe,x_f,y_f,z_f
